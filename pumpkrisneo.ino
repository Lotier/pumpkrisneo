...
void drawFixedPieces() {
  for(int i = 0; i < BOARD_HEIGHT; i++) {
    for(int j = 0; j < BOARD_WIDTH; j++) {
      if (board[i][j] != EMPTY) {
        if (i % 2 == 0) { // Normal Row
          leds[i * BOARD_WIDTH + j] = pieces[board[i][j]].color;
        } else { // Inversed Row
          leds[i * BOARD_WIDTH + (BOARD_WIDTH - 1 - j)] = pieces[board[i][j]].color;
        }
      }
    }
  }
...
void drawActivePiece() {
  for (int thisPixel=0; thisPixel < 4; thisPixel++){
    ...
    leds[pixelY * BOARD_WIDTH + adjustedX] =  pieces[activeShape].color;
  } 
}
